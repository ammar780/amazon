...
function AuthProvider({ children }) {
  ...
  return React.createElement(AuthCtx.Provider, { value: { user, completeLogin, logout } }, children);
}
...
function LoginPage() {
  var auth = useAuth();
  var [step, setStep] = useState('login'); // login | mfa_setup | mfa_verify | change_password
  var [email, setEmail] = useState('');
  var [pass, setPass] = useState('');
  var [err, setErr] = useState('');
  var [loading, setLoading] = useState(false);
  var [tempToken, setTempToken] = useState('');
  var [qrCode, setQrCode] = useState('');
  var [mfaSecret, setMfaSecret] = useState('');
  var [mfaCode, setMfaCode] = useState('');
  var [oldPass, setOldPass] = useState('');
  var [newPass, setNewPass] = useState('');
  var [pwExpired, setPwExpired] = useState(false);
  ...
  function doLogin(e) {
    ...
    api.login({ email: email, password: pass }).then(function(r) {
      ...
      // Direct token = no MFA, old flow fallback
      if (r.token && r.user) {
        auth.completeLogin(r);
        return;
      }

      // MFA setup required (first time)
      if (r.mfa_setup_required) {
        setTempToken(r.temp_token);
        setPwExpired(r.password_expired);
        setStep('mfa_setup');
        // Fetch QR code
        api.mfaSetup({ temp_token: r.temp_token }).then(function(s) {
          if (s && s.qr) { setQrCode(s.qr); setMfaSecret(s.secret); }
          else if (s && s.error) { setErr(s.error); }
        });
        return;
      }

      // MFA verify required (already set up)
      if (r.mfa_required) {
        setTempToken(r.temp_token);
        setPwExpired(r.password_expired);
        setStep('mfa_verify');
  ...
  function doMfaVerify(e) {
    e.preventDefault(); setErr(''); setLoading(true);
    api.mfaVerify({ temp_token: tempToken, code: mfaCode }).then(function(r) {
      setLoading(false);
      if (!r) return;
      if (r.error) { setErr(r.error); setMfaCode(''); return; }
      if (r.token && r.user) {
  ...
  function doChangePassword(e) {
    e.preventDefault(); setErr(''); setLoading(true);
    api.changePassword({ old_password: oldPass, new_password: newPass }).then(function(r) {
      setLoading(false);
      if (!r) return;
      if (r.error) { setErr(r.error); return; }
      if (r.ok) { setPwExpired(false); setStep('login'); }
    }).catch(function(e) { setLoading(false); setErr(e.message); });
  }
  ...
  if (step === 'change_password') {
    return (
      <div className="login-wrap"><div className="login-box"><div className="login-card">
        <h1>Password Expired</h1>
        <p className="lsub">Your password must be changed (365-day policy)</p>
        {err && <div className="err">{err}</div>}
        <form onSubmit={doChangePassword}>
          <div className="fg"><label>Current Password</label><input type="password" value={oldPass} onChange={function(e){setOldPass(e.target.value)}} required /></div>
          <div className="fg"><label>New Password</label><input type="password" value={newPass} onChange={function(e){setNewPass(e.target.value)}} required placeholder="Min 12 chars, upper+lower+number+special" /></div>
          <button className="btn btn-gold" type="submit" disabled={loading} style={{marginTop:8}}>{loading ? 'Changing...' : 'Change Password'}</button>
        </form>
      </div></div></div>
    );
  }
  ...
  if (step === 'mfa_setup') {
    return (
      <div className="login-wrap"><div className="login-box"><div className="login-card">
        <h1>Set Up MFA</h1>
        <p className="lsub">Scan with Google Authenticator or Authy</p>
        {err && <div className="err">{err}</div>}
        {qrCode && <div style={{margin:'16px 0'}}><img src={qrCode} alt="MFA QR" style={{width:200,height:200,borderRadius:8,background:'#fff',padding:8}} /></div>}
        {mfaSecret && <p style={{fontSize:10,color:'var(--t3)',marginBottom:16,fontFamily:'IBM Plex Mono',wordBreak:'break-all'}}>Manual key: {mfaSecret}</p>}
        <form onSubmit={doMfaVerify}>
          <div className="fg"><label>Enter 6-digit Code</label><input value={mfaCode} onChange={function(e){setMfaCode(e.target.value)}} required placeholder="000000" maxLength="6" style={{textAlign:'center',fontSize:24,letterSpacing:8,fontFamily:'IBM Plex Mono'}} autoFocus /></div>
          <button className="btn btn-gold" type="submit" disabled={loading} style={{marginTop:8}}>{loading ? 'Verifying...' : 'Verify & Activate MFA'}</button>
        </form>
      </div></div></div>
    );
  }

  // MFA VERIFY SCREEN (returning user)
  if (step === 'mfa_verify') {
    return (
      <div className="login-wrap"><div className="login-box"><div className="login-card">
        <h1>MFA Verification</h1>
        <p className="lsub">Enter code from your authenticator app</p>
        {err && <div className="err">{err}</div>}
        <form onSubmit={doMfaVerify}>
          <div className="fg"><label>6-Digit Code</label><input value={mfaCode} onChange={function(e){setMfaCode(e.target.value)}} required placeholder="000000" maxLength="6" style={{textAlign:'center',fontSize:24,letterSpacing:8,fontFamily:'IBM Plex Mono'}} autoFocus /></div>
          <button className="btn btn-gold" type="submit" disabled={loading} style={{marginTop:8}}>{loading ? 'Verifying...' : 'Verify'}</button>
        </form>
        <p style={{fontSize:10,color:'var(--t3)',marginTop:16,cursor:'pointer'}} onClick={function(){setStep('login');setErr('');setMfaCode('')}}>Back to login</p>
      </div></div></div>
    );
  }
    ...
    <div className="login-wrap">
      <div className="login-box">
        <div className="login-card">
          ...
          <p style={{fontSize:10,color:'var(--t3)',marginTop:20}}>Protected by MFA + 365-day password policy</p>
        </div>
      </div>
    </div>
  );
